...
num_simulations = 5000; % number of Monte Carlo runs
...
while i < num_simulations
    ...
    wind_direction = mean_direction + std_direction * randn; % Normal sampling for wind direction at the top
...
% Direction change model
    ...
    % active ARES control corrects the parachute orientation at every time step to point towards the target coordinates
    if mod(time_incr, 60)
        parachute_orientation = atan2(target(2) - position_y, target(1) - position_x);
        parachute_orientation = mod(parachute_orientation, 2*pi);
        glide_u = glide_velocity * cos(parachute_orientation); % x-component of glide
       glide_v = glide_velocity * sin(parachute_orientation); % y-component of glide
    end
...
ylabel('Y (mi)')           
zlabel('Z (ft)')          
title('3D Trajectory with partial ARES control')
scatter(target(1)/5280,target(2)/5280,'filled')
hold off
...
grid minor
xlabel('altitude')
ylabel('wind speed distribution')
title('wind speed by altitude')

% 2D Heatmap of landing positions
figure;
scatter(landing_positions(:,1)./5280, landing_positions(:,2)./5280, 15, 'filled');
title('2D Landing Dispersion Heatmap with Wind Effects');
...
hold off;
grid minor
title('Distribution of Landing Distance with partial ARES Control');
xlabel('Distance (mi)')
